...
class SchedulerService:
    ...
    async def start(self):
        ...
        try:
            ...
            # Job 1: Price & Score every 5 min, not at :00 and :30
            ...
            # Job 3: Tournament Lifecycle (combined)
            self.scheduler.add_job(
                func=self._check_tournaments_lifecycle,
                trigger=CronTrigger(minute='0,30', second=30),
    ...
    async def _price_and_score_job(self):
        """Price update and score calculation with job lock."""
                ...
                try:
                    ...
                    async with AsyncSessionLocal() as score_db:
                        ...
                        if tournament:
                            ...
                            logger.info("   No active tournament - writing zero scores")
                    ...
                    if tournament:
                        ...
                        logger.info("[3/4] No ongoing tournament - skipping leaderboard update")
    ...
    async def _check_tournaments_lifecycle(self):
        """Handle full tournament lifecycle with job lock."""
        async with AsyncSessionLocal() as db:
    ...
    async def _check_tournaments_to_start(self):
        ...
        try:
            ...
            for tournament in registration_tournaments:
                try:
                    logger.info(
                        f"Starting tournament #{tournament.tournament_number} "
    ...
    async def _check_tournaments_to_finish(self):
        """Finish tournaments, soft-delete expired cards/packs, grant new packs."""
        try:
            async with AsyncSessionLocal() as db:
                now = datetime.now(timezone.utc)
                
                # 1. Find tournaments to finish
                result = await db.execute(
                    select(Tournament).where(
                        and_(
                            Tournament.status == TournamentStatus.ONGOING,

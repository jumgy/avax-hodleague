...
async def get_pack_types(
    ...
    query = db.query(PackType)
    

    if id is not None:
        query = query.filter(PackType.id == id)
    if name:
        query = query.filter(PackType.name.ilike(f"%{name}%"))
    if description:
        query = query.filter(PackType.description.ilike(f"%{description}%"))
    if cards_per_pack is not None:
        query = query.filter(PackType.cards_per_pack == cards_per_pack)
    if cards_per_pack_from is not None:
        query = query.filter(PackType.cards_per_pack >= cards_per_pack_from)
    if cards_per_pack_to is not None:
        query = query.filter(PackType.cards_per_pack <= cards_per_pack_to)
    if price_from is not None:
        query = query.filter(PackType.price >= price_from)
    if price_to is not None:
        query = query.filter(PackType.price <= price_to)
    if currency:
        query = query.filter(PackType.currency == currency)
    if supply_from is not None:
        query = query.filter(PackType.supply >= supply_from)
    if supply_to is not None:
        query = query.filter(PackType.supply <= supply_to)
    if is_active is not None:
        query = query.filter(PackType.is_active == is_active)
    if available_from_after:
        query = query.filter(PackType.available_from >= available_from_after)
    if available_from_before:
        query = query.filter(PackType.available_from <= available_from_before)
    if available_until_after:
        query = query.filter(PackType.available_until >= available_until_after)
    if available_until_before:
        query = query.filter(PackType.available_until <= available_until_before)
    if created_from:
        query = query.filter(PackType.created_at >= created_from)
    if created_to:
        query = query.filter(PackType.created_at <= created_to)
    if updated_from:
        query = query.filter(PackType.updated_at >= updated_from)
    if updated_to:
        query = query.filter(PackType.updated_at <= updated_to)
...
async def get_pack_type(
    ...
    pack = db.query(PackType).filter(PackType.id == pack_type_id).first()
...
async def create_pack_type(
    ...
    existing = db.query(PackType).filter(PackType.name.ilike(data.name)).first()
    if existing:
        raise HTTPException(status_code=400, detail=f"Pack type name '{data.name}' already exists")
...
async def update_pack_type(
    ...
    if data.supply is not None:
        active_purchases = db.query(UserPack).filter(

...
def get_tournaments_list(
    ...
    try:
        ...
        tournaments = query.order_by(Tournament.tournament_number.desc()).offset(offset).limit(limit).all()
        ...
        for tournament in tournaments:
            # Count participants
            participants_count = db.query(func.count()).select_from(TournamentDeck).filter(
            ...
            is_registered = False
            if user_id:
                deck = db.query(TournamentDeck).filter(
                    TournamentDeck.tournament_id == tournament.id,
